...
QUEUE = [("karma_nirnaya", "blocks_knr.json", "knr_all")]
...
def run(slug, blocks, stem):
    log(f"=== {slug} ===")
    want = wanted(blocks)
    pass   # nothing to reuse: karma_nirnaya has never been rendered
    launch(slug, stem)
...
if __name__ == "__main__":
    log(f"karma_nirnaya (commentary only) render starting — {len(NODES)} cards (box3 down; .167 used despite NVML)")
