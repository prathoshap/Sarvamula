...
QUEUE = [("nyaya_vivarana", "blocks_nvv.json", "nvv_all")]
...
def run(slug, blocks, stem):
    log(f"=== {slug} ===")
    want = wanted(blocks)
    pass   # nothing to reuse: nyaya_vivarana has never been rendered
    launch(slug, stem)
...
if __name__ == "__main__":
    log(f"nyaya_vivarana (commentary only) render starting — {len(NODES)} cards (box3 down; .167 used despite NVML)")
