...
(function () {
  ...
  window.anuLocScope = function (slug) { locScope = slug; if (locState) locate(locState.term, locState.concept); };
  ...
  function locate(term, concept) {
    ...
    const sc = locScope ? ' AND work=?' : '', scA = locScope ? [locScope] : [];   // grantha-wise scope
