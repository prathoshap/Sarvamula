...
// search normalization (norm) — VOWEL-PRESERVING, MULTI-SCRIPT. MUST stay byte-identical to
// norm() in build_db.py (which builds text_skel). Handles all Indic scripts (Kannada, Telugu,
// Malayalam, etc.) by converting to Devanāgarī first via BhagNorm.scriptToDev. Keeps vowels
// (long→short), folds aspirates/retroflex/sibilants, drops anusvāra/visarga, vocalic-ṛ → "ri".
...
function norm(s){s=s||'';
  // Convert any Indic script to Devanagari first (Kannada, Telugu, Malayalam, etc.)
  // BhagNorm.scriptToDev uses Unicode offsets to convert all Indic scripts to Devanagari
  if(window.BhagNorm) s=BhagNorm.scriptToDev(s);
...
function getPrasthanaWorks(prasthanaIdx){
  // Return the list of grantha slugs in a prasthana (identified by its index in GROUPS)
  if(prasthanaIdx<0 || prasthanaIdx>=GROUPS.length) return [];
  return GROUPS[prasthanaIdx][2];
}
function scopeSelect(id,cur,onchg){
  const works=q("SELECT slug,title FROM works ORDER BY title");
  const isGroup=cur&&cur.startsWith('prasthana:');

  // Prasthana group options - use a dropdown select
  let groupOpts=`<select name="scope-${id}" onchange="${onchg}(this.value);" class="work-select">
    <option value="">All groups</option>`;
  for(let i=0;i<GROUPS.length;i++){
    const [dev,en]=GROUPS[i];
    const val='prasthana:'+i;
    groupOpts+=`<option value="${val}" ${cur===val?'selected':''}>${en}</option>`;
  }
  groupOpts+='</select>';

  // Individual grantha options - use a dropdown select
  let workOpts=`<select name="scope-${id}" onchange="${onchg}(this.value);" class="work-select">
    <option value="">All works</option>`;
  for(const w of works){
    workOpts+=`<option value="${w.slug}" ${w.slug===cur?'selected':''}>${w.title}</option>`;
  }
  workOpts+='</select>';

  return `<div class="sscope">
    <div class="sscope-row">
      <label class="sscope-label">Search by Groups:</label>
      ${groupOpts}
    </div>
    <div class="sscope-row">
      <label class="sscope-label">Search by Works:</label>
      ${workOpts}
    </div>
  </div>`;
}
...
function renderSearch(term,page){
  ...
  if(searchScope){
    if(searchScope.startsWith('prasthana:')){
      const prasIdx=parseInt(searchScope.split(':')[1]);
      const prasWorks=getPrasthanaWorks(prasIdx);
      if(prasWorks.length>0){
        const placeholders=prasWorks.map(()=>'?').join(',');
        where=`(${where}) AND work IN (${placeholders})`;
        args=[...args,...prasWorks];
      }
    }else{
      where=`(${where}) AND work=?`;  // grantha-wise scope
      args=[...args,searchScope];
    }
  }
  ...
  const scopeDesc=searchScope?(searchScope.startsWith('prasthana:')?GROUPS[parseInt(searchScope.split(':')[1])][1]:titles[searchScope]||searchScope):'';
  if(!searchTotal){view().innerHTML=scope+'<div id="status">no matches'+(scopeDesc?' in '+scopeDesc:'')+'</div>';return;}
